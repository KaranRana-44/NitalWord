...
public class NitalTester {
    ...
    public static boolean isNital(int[][] square){
        int sl= square.length;
        int[]a1=square[0];
        int[]c1=getColumn(square, 0);
        if(containsRepeats(a1)){return false;}
        for(int j=0;j<sl; j++){
            if(!hasAllValues(a1, square[j])){return false;}
            if(!hasAllValues(c1, getColumn(square,j))){return false;}
        }
